...
Dublin since 2010, ingested monthly from Ireland's official
[PSRA Residential Property Price Register](https://www.propertypriceregister.ie),
stored in Supabase, and shown on a small dark-themed dashboard with type-ahead
search by address, Eircode, or postal district.
...
```
PPR CSV (monthly)  →  GitHub Action (scripts/ingest.mjs)  →  Supabase Postgres  →  static dashboard (Cloudflare Pages)
```
...
- **Ingestion**: `scripts/ingest.mjs` downloads the CSV, decodes it (source is
  ...
  or address text, and upserts into Supabase keyed on an md5 hash of
  `(date, address, price)` — safe to re-run.
- **Scheduling**: `.github/workflows/ingest.yml` runs it on the 1st of every
  month, plus on-demand via `workflow_dispatch`.
...
- **Dashboard**: `index.html` / `style.css` / `app.js` — no build step,
  ...
  anon key (safe — reads only, enforced by RLS).
